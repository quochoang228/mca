...
extension StringExtension on String? {
  ...
  bool validatePhone() {
    if (hasMatch(this, Patterns.phone) && (this ?? '').length == 10) {
      if (Patterns.phonePrefix.firstWhere((element) => (this ?? '').startsWith(element)).isNotEmpty) {
        return true;
      } else {
        return false;
      }
  ...
  //     return false;
  //   } else {
  //     return hasMatch(this, Patterns.password);
  //   }
  // }
  ...
  String getHeadingCharater() {
    if ((this ?? '').isEmpty) {
      return '';
    }
    var splitName = (this ?? '').split(' ');
    return '${splitName.first[0]}${splitName[splitName.length - 1][0]}';
  }
  ...
  bool isAlpha() => Patterns.alphaRegExp.hasMatch(validate());
  ...
  double toDouble({double defaultValue = 0.0}) {
    ...
    try {
      double? result = NumberFormat.decimalPattern(Intl.systemLocale).parse(this!) as double?;
