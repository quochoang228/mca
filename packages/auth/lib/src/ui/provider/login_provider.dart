...
final loginProvider =
    StateNotifierProvider.autoDispose<LoginProvider, DataSate<User>>(
  (ref) => LoginProvider(ref),
);

class LoginProvider extends StateNotifier<DataSate<User>> {
        ...
        result.when(
          success: (data) async {
            if (data.status == 201 && data.data != null) {
              ...
              // var parseToken =
              //     jsonEncode(JwtDecoder.decode(data.data!.token ?? '')["sub"])
              //         .replaceAll("\\", "")
              //         .replaceAll("\"{", "{")
              //         .replaceAll("}\"", "}");

              // User.fromJson(jsonDecode(parseToken));
