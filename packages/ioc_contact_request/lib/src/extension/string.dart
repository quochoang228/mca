...
RegExp _alphaAndSpace = RegExp(r'^[a-zA-Z ]+$');

RegExp validTaxCode = RegExp(r'^\d{10}(-\d{3})?$');

List<String> _namePrefix = [
  'C.',
  'E.',
  'C',
  'E',
...
extension StringParsing on String {
  ...
  String removeEscapeCharacter() {
    if (isNotEmpty) {
      return trim().replaceAll('\n', "<br/>").replaceAll(r'\', r'\\').replaceAll(r'"', r'\"').replaceAll("[\u0000-\u001F]", "");
    }
    return this;
  }

  String revertEscapeCharacter() {
    if (isNotEmpty) {
  ...
  bool get isNotEmptyString {
    var stringTrim = (this ?? '').trim();
    return stringTrim.isEmptyOrNull == false && stringTrim != null.toString();
  }
  ...
  bool get isAlphaAndSpace {
    return _alphaAndSpace.hasMatch(removeDiacritics());
  }
  ...
  bool get validPhoneNumber {
    bool isValid = isNotEmptyString && validatePhone();
    if (isValid) {
      isValid = _phonePrefix
          .firstWhere(
  ...
  bool get validHotline {
    bool valid = hotline.firstWhere((prefix) => startsWith(prefix), orElse: () => '').isNotEmpty;
    return valid;
  }

  bool get validPhoneNumber11 {
    bool isValid = isNotEmptyString && validatePhone();
    if (isValid) {
      isValid = _phonePrefix11
          .firstWhere(
            (prefix) => startsWith(prefix),
            orElse: () => '',
          )
  ...
  bool get isValidEmail {
    ...
    if (elements.isNotEmpty) {
      RegExp emailPrefix = RegExp(r'^[a-zA-Z0-9._]+$');
      String prefix = elements[0];
      return emailPrefix.hasMatch(prefix);
    }
    return false;
  }

  bool get isNameCustomer {
    if (isEmpty) {
      return false;
    }
    List<String> elements = split(' ');
    if (elements.isNotEmpty) {
      String prefix = elements[0];
      return _namePrefix.contains(prefix);
    }
    return false;
  }

  bool get isUpperCaseText {
...
extension BoolParing on String {
  bool parseBool() {
...
extension DoubleParsing on String {
  num parseToDouble() {
    try {
      return NumberFormat.currency(locale: 'vi').parse(this);
      // return NumberFormat.currency(locale: 'vi').parse(this);
    } catch (e) {
      return 0;
    }
